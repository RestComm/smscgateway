...
=== Memory Settings (Wildfly 10)

You should fine tune the JVM memory settings based on your needs but we recommend you allocate a minimum of 3 GB for initial and maximum heap size. 4 GB is recommended.
These settings are specified in the file [path]_{this-folder}-smscgateway-version>/wildfly-10.1.0.Final/bin/standalone.conf by adding following parameters to JAVA_OPTS
...
.Procedure for Wildfly 10
* open file wildfly-10.1.0.Final/standalone/configuration/standalone.xml in the text editor
* find "bean name="Mtp3UserPart"" mbean definition
* update parameter "deliveryMessageThreadCount" to some value like "processor count * 2"
...
[[_logging_wildfly]]
== Configuring log4j Logging Service (Wildfly 10)

{this-platform} {this-application} uses `Apache log4j` for logging.
If you are not familiar with the `log4j` package, you can read more about it at the Jakarta http://jakarta.apache.org/log4j/[website].

Logging is controlled from a central configuration file located at [path]{this-folder}-smscgateway-<version>/wildfly-10.1.0.Final/standalone/configuration/standalone.xml.
This file defines a set of appenders specifying the log files, what categories of messages should go there, the message format and the level of filtering.
For more details, please refer to Wildfly 10 "Logging Configuration" guide https://docs.jboss.org/author/display/WFLY10/Logging+Configuration
...
This chapter introduces a description how Telestax SMSC GW Active-Active can be configured. See below an example of a configuration with 2 FEP and 2 SMSC GW nodes.

image::images/SMSC-FEP-schema.png[]

Two SMSC GWs ("SMSC 1" and "SMSC 2") can be deployed at different physical servers (server 1 and server 2). Front-End-Processors ("FEP 1" and "FEP 2") plays a role of SS7 load balancers. "FEP 1" and "FEP 2" can be deployed to same physical servers as "SMSC 1" and "SMSC 2" (server 1 and server 2). "FEP 1" and "FEP 2" are connected by SS7 links to remote STP node(s).

A remote originated TCAP dialogs are loadshared by FEP nodes between existing SMSC nodes. Then FEP nodes are responsible for resending of TCAP dialog messages to a proper SMSC node. This guarantees that all SS7 dialogs will be processed by a same SMSC GW node and SMSC GW nodes does not demand processing of SS7 dialogs from another node. This means that SMSC GW nodes still process messages independently. FEP nodes are needed to delivery of SS7 messages to a proper SMSC GW node.

When you use several FEP / SMSC GW nodes in a way that is described at a picture you can turn off any of FEP / SMSC GW node for maintanance or for other reasons. All traffic then will be processed by alive nodes. Each SMSC GW stores all pending messages (for storeAndForward and forwardAndStore modes) in it's own cassandra KeySpace. Another SMSC GW node does not have access to such stored messages. This means that we will need to turn a SMSC GW node on in order to pending messages will be delivered.

SMSC GW supports of a graceful shutdown procedure. See more in a chapter <<_gsd_settings>>. A graceful shutdown procedure is initiated by a GUI / CLI interface. SMSC GW will stop of loading new messages from a database and will make a server shutdown when all message delivering processes have completed. FEP nodes will prevent of sending new external dialogs to a SS7 node that is gracefully shutting down. SMSC GW node even after a graceful shutdown start will aceept and process of incoming messages from other interfaces (SMPP, SIP, HTTP). A user must stop of sending of messages via these interfeces before of a graceful shutdown start.

For a smooth support of HR (Home Routing) and AlertCerviceCenter operations between SMSC GW nodes an extra cassandra KeySpace is introduced as an addition to a dedicated cassandra KeySpace (default name is "Shared").
This new clustering KeySpace is used for cache of correlationId values and data for HR procedure and exchanging of AlertCerviceCenter requests between SMSC GW servers (for MT SS7 procedure).
If you use several SMSC GW nodes then you need to turn on an option `In cluster`, prepare a new empty cassandra KeySpace (a single instance for both/all SMSC GW nodes) and configure this KeySpace in `Shared Database Configuration` section.

Here is a script example for settings a shared KeySpace options.

[source]
----
smsc set shareddbhosts <IP>
smsc set shareddbport 9042
smsc set shareddbkeyspace Shared
smsc set shareddbuser cassandra
smsc set shareddbpass cassandra

smsc set incluster true
----

You can use also web GUI `http://localhost:8080/smsc-management/` - "Server settings" - "Clustering". See more info in the chapter <<_smsc_clustering_settings>>.

When working of {this-application} we need to have both dedicated and shared cassandra KeySpaces up. We may need to bring cassandra databases down for maintenance. A dedicated KeySpace may be turned down with a corresponded {this-application} server.
If we need to turn down a database with a shared keyspace and some SMSC GWs are up that time then we need to follow this order:

* turn `InCLuster` option to `false`
* stop a database with shared KeySpace
* start a database with shared KeySpace
* turn `InCLuster` option to `true`

Turning `InCLuster` to `false` or `true` can be done by a web GUI or by CLI commands

[source]
----
smsc set incluster false

smsc set incluster true
----

See for details <<_in_cluster>>.

We need to make special configuring procedures of SS7 part before SMSC GW / FEP will be able to work in a load-balancing mode. This configuring is described in a manual "Restcomm Load Balancer User Guide", check a chapter "Integration Load Balancer SS7 (FEP) with USSD and SMSC servers".
...
==== Routing desision when we have several service providers and operators

An example of connection schema for connecting to operators and clients:

image::images/SMS-routing-ex.png[]

We will do routing based on message destination addresses mask. For example the algorithm may be:

* all messages with destination addresses starting with "1*" will be routed to client 1 (with networkId 1)
* all messages with destination addresses starting with "2*" will be routed to client 2 (with networkId 2)
* all other messages will be routed to an operator (with etworkId 3)

===== Routing based on destination address mask

Example algorithm: we have 3 networkId areas (1, 2 and 3) for client / agents.

[source]
----
smsc mproc add 1 destdigmask ^1[0-9].* newnetworkid 1
smsc mproc add 2 destdigmask ^2[0-9].* newnetworkid 2
smsc mproc add 3 destdigmask ^(?!1|2).* newnetworkid 3
----

===== Routing based on destination address mask with usage of intermediate networkId

Example algorithm: we have 3 networkId areas (1, 2 and 3) for client / agents. We use intermediate networkId areas (101, 102, 103 for temporary parking routed messages) for simpler formalizing.

[source]
----
smsc mproc add 1 destdigmask ^1[0-9].* newnetworkid 101
smsc mproc add 2 destdigmask ^2[0-9].* newnetworkid 102
smsc mproc add 101 networkidmask 1 newnetworkid 103
smsc mproc add 102 networkidmask 2 newnetworkid 103
smsc mproc add 103 networkidmask 3 newnetworkid 103
smsc mproc add 201 networkidmask 101 newnetworkid 1
smsc mproc add 202 networkidmask 102 newnetworkid 2
smsc mproc add 203 networkidmask 103 newnetworkid 3
----

===== Routing when a client belongs to an operator

Example algorithm: we have operator 1 (networkId==1) and a client 1 (networkId==101) and all messages from client 1 will be routed to soperator 1 and vice versa, we have operator 2 (networkId==2) and a client 2 (networkId==102) and all messages from client 2 will be routed to soperator 2 and vice versa.

[source]
----
smsc mproc add 1 originnetworkidmask 1 newnetworkid 101
smsc mproc add 2 originnetworkidmask 2 newnetworkid 102
smsc mproc add 3 originnetworkidmask 101 newnetworkid 1
smsc mproc add 4 originnetworkidmask 102 newnetworkid 2
----

==== Routing decision based on SRI request results
...
| sriremotesccpgtprefix | Arrival Trigger / <sri remote sccp gt prefix> | sriremotesccpgtprefix will be added to the beginning of a message SCCP called party address to route SriForSM request to. | "-1". This means that Sccp called party address will not be changed.
...
| removedestdigprefix | Arrival trigger / <value> | First <value> digits of destination address will be removed. | "0". This means that no action will be taken.
|===
...
* User must compile java code and create a jar library.
  Copy the created jar into the folder [path]_{this-folder}-smscgateway-<version>/wildfly-10.1.0.Final/modules/system/layers/base/org/mobicents/smsc/bootstrap/main.
* You need to update [path]_{this-folder}-smscgateway-<version>/wildfly-10.1.0.Final/standalone/configuration/standalone.xml config file.
+
In the "bean name="SmscManagement"" section - property <property name="MProcRuleFactories"> - update the line <inject bean="MProcRuleFactoryDefault"/> (that already contains default mproc rules implementing) - add into line your new created mproc rules factory class using `,` as a separator (several classes can be added):
+
[source,xml]
----
...
Pay also attention that SMSC GW in Home Routing mode usually plays "HLR role" and listens an extra SSN (usually SSN 6 as additionto SSN 8 for other functionality). These parameters are configured in `jboss-beans.xml` for JBOSS 5 case and `standalone.xml` for Wildfly 10 case file for TCAP stack. You can find below the example of configuring of two listerning SSNs (8 and 6 in the example). This configuration is a default configuration out of box.
...
.Content for Wildfly 10

[source,xml]
----
	<mbean class="org.restcomm.protocols.ss7.tcap.TCAPStackImpl" enabled="true" name="TcapStackMap" reflection="false" type="TcapStack">
...
== SMS Routing By SMPP Optional TLV Fields
Telscale SMSC has supprted special routing by using special SMPP TLV field, there are 4 fiels (you need to fill these tag in your SMPP client and send to Telscale SMSC as a submit_sm or deliver_sm), in this case, Telscale SMSC will not send SRIforSM and use these values for sending MtForwardSM:

* TAG_IMSI: value 1403, it's IMSI of destination, C-Octec String and mandatory
* TAG_NNN_DIGITS: value 1404, it's Network Node Number (MSC/VLR GT numbers), C-Octec String and mandatory
* TAG_NNN_TON: value 1405, it's TON number, Interger and optional
* TAG_NNN_NPI: value 1406, it's NPI number, Interger and optional

If TAG_IMSI or TAG_NNN_DIGITS has empty or bad value, SMSC will ignore other TAGs and accept SMS as normal SMS.
If TAG_NNN_TON or TAG_NNN_NPI has empty or bad value, SMSC will fill with 1
This feature only support by Telscale SMSC version 
...
SMSC GW has two configurable parameters for decoding / encodoing, one for GSM 7 encoding and another for UCS2 encoding (SMPP Encoding for GSM7 / SMPP Encoding for UCS2).
See following chapter for configuring details: <<_smpp_encoding_for_ucs2>> for server level configuring and parameters `smppEncodingForGsm7` and `smppEncodingForUCS2` for configuring per ESME in "External Short Messaging Entities (ESMEs)" chapter of "Restcomm SMPP Extensions User Guide". 
...
But sometimes SMSC GW plays a role of a transit node and SMSC GW customers interest in delivery receipts from upper  SMSC GW nodes (nodes that deliver messages directly to customers) an not from a transit SMSC GW itself. {this-platform} SMSC GW can play such role and this case is more complicated. SMSC GW must accept delivery receipts from upper SMSC GW nodes, replace messageId values in these receipts by a messageId which SMSC GW used in submit_sm_response to a lower node that has sumbitted an original message and sends an updated receipt to an original ESME.

Another possible option - SMSC GW can replace an error code in an incoming delivery receipt (DLR) to another error code (that may fit to SMSC GW error codes set). This functionality is configurable per ESME and allows us to have an error code translation table per ESME.
Delivery Error Translation tables have a name and a list of matching between original error codes and translated error code like `*:012,101:014,102:015`. In the example code 101 will be translated to 014, 102 to 015 and all other codes to 012 (this part is optional, if no syntax like `*:012` was added then other error codes will not be changed).   

We call such process as a delivery receipt transit.

To achieve this functionality we must make following configuring:

* Switch the option <<_deliveryreceiptslevel>> to "FAILURE" (by this we prevent SMSC GW from generating of its own delivery receipts for success delivery and generate it only for delivery errors)
...
* If we need of DLR error code changing then we need to configure Delivery Error Translation tables by <<_det_tables>> and specify a needed Delivery Error Translation table by specifying of option "detRule" for ESME via which a DLR has come to SMSC GW.  
...
* from <smsc gw root folder>/resource/ocs/smsc-part to SMSC GW <jboss>/server/default/deploy/{this-folder}-diameter-mux-6.2.0.GA.sar/config for JBOSS 5 server and <smsc gw root
folder>/extra/restcomm-diam/modules/lib/main/config for Wildfly 10 server
...
This file is already copied to SMSC GW into Simulator profile for JBOSS 5 case. It is not performed for Wildfly 10 server because Wildfly 10 has same comfiguring files placed for diameter module for all profiles.
...
{this-platform} {this-application} is configured to generate CDR in a plain text files `cdr.log` and `cdr_detailed.log` located at [path]_{this-folder}-smscgateway-<version>/jboss-5.1.0.GA/server/<profile>/log/cdr.log_ for JBOSS 5 server and [path]_{this-folder}-smscgateway-<version>/wildfly-10.1.0.Final/standalone/log for Wildfly 10 server and also detailed CDR in Cassandra tables `MESSAGES_yyyy_mm_dd`.
