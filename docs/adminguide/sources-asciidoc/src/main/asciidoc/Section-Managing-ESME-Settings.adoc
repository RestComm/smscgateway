...
SYNOPSIS
	...
	esme-error-counters-enabled <null | true | false> esme-maintenance-counters-enabled 
	<null | true | false> session-error-counters-enabled <null | true | false>
...
PARAMETERS
	...
	esme-error-counters-enabled - Default value is null.
		          When this property is unset (set to null), the corresponding
		          global setting will be applied to this esme.
		          Otherwise, if it is set to true, statistic of errors
		          occurred in SMSC related to this ESME will be gathered.
		          
		          This property overrides global SMSC property which enables
		          stats generating for every esme.

  	esme-maintenance-counters-enabled - Default value is null.
		          When this property is unset (set to null), the corresponding
		          global setting will be applied to this esme.
		          Otherwise, if it is set to true, SMSC will expose Request
		          and Response queue sizes for this ESME.
		          
		          This property overrides global SMSC property which enables
		          queue size exposure for every esme.

  	session-error-counters-enabled - Default value is null.
		          When this property is unset (set to null), the corresponding
		          global setting will be applied to this esme.
		          Otherwise, if it is set to true, statistic of errors
		          occurred in SMSC per every session establised with this
		          ESME will be gathered.
		          
		          This property overrides global SMSC property which enables
		          stats generating for every session.		
