...
. The main panel will display the existing Settings, segregated into  horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
...
. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' in the left panel.
. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
  Switch to the 'Properties' tab in the GUI.
...
[[_set_globalerrorcountersenabled]]
=== SMSC Global Error Counters

[[_set_globalerrorcountersenabled_cli]]
==== Using CLI

You can set the value 'SMSC Global Error Counters Enabled' to true/false by entering the command `smsc set globalerrorcountersenabled`.
You can verify new value then by issuing the command `smsc get globalerrorcountersenabled` which will display the value set for this property.

----

Name
        smsc set globalerrorcountersenabled

SYNOPSIS
        smsc set globalerrorcountersenabled <true | false>

DESCRIPTION
				The SMSC can be configured to calculate total number of errors occurred in SMSC in different areas during certain period of time.
				If we set globalerrorcountersenabled to true then every SMSC error will be counted towards the value of corresponding counter.
				The logical units for which global counters will be calculated are: Scheduler, MProc, SMPP IN and SMPP OUT, SIP IN and SIP OUT, MAP IN and MAP OUT, HTTP IN.
				If globalerrorcountersenabled is set to false then errors won't be counted globally while they still can be calculated per cluster(s), per esme(s), per sessionId or per mProcRuleId where applicable.
				Default value: false

EXAMPLES
	smsc set globalerrorcountersenabled false
----

[[_globalerrorcountersenabled_gui]]
==== Using GUI

.Procedure: Set 'globalerrorcountersenabled'  using the GUI
. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
  Click on the 'Counters'..
. You can specify if the global error counters should be enabled in SMSC by choosing 'true' for the field 'Global Error Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
. You must click on the button 'Apply Changes' at the top of the window to save your settings.
  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


	[[_set_clustererrorcountersenabled]]
	=== SMSC Cluster Error Counters

	[[_set_clustererrorcountersenabled_cli]]
	==== Using CLI

	You can set the value 'SMSC Cluster Error Counters Enabled' to true/false by entering the command `smsc set clustererrorcountersenabled`.
	You can verify new value then by issuing the command `smsc get clustererrorcountersenabled` which will display the value set for this property.

	----

	Name
	        smsc set clustererrorcountersenabled

	SYNOPSIS
	        smsc set clustererrorcountersenabled <true | false>

	DESCRIPTION
					The SMSC can be configured to calculate total number of errors occurred in SMSC per cluster in certain logical unit.
					Cluster counters are only available in Scheduler, SMPP IN and SMPP OUT logical units.
					If we set clustererrorcountersenabled to true then every SMSC error will be counted towards the value of corresponding cluster counter.
					If clustererrorcountersenabled is set to false then errors won't be counted per cluster.
					Default value: false

	EXAMPLES
		smsc set clustererrorcountersenabled false
	----

	[[_clustererrorcountersenabled_gui]]
	==== Using GUI

	.Procedure: Set 'clustererrorcountersenabled'  using the GUI
	. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
	. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
	  Click on the 'Counters'..
	. You can specify if the cluster error counters should be enabled by choosing 'true' for the field 'Cluster Error Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
	. You must click on the button 'Apply Changes' at the top of the window to save your settings.
	  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


		[[_set_esmeerrorcountersenabled]]
		=== SMSC ESME Error Counters

		[[_set_esmeerrorcountersenabled_cli]]
		==== Using CLI

		You can set the value 'SMSC ESME Error Counters Enabled' to true/false by entering the command `smsc set esmeerrorcountersenabled`.
		You can verify new value then by issuing the command `smsc get esmeerrorcountersenabled` which will display the value set for this property.

		----

		Name
		        smsc set esmeerrorcountersenabled

		SYNOPSIS
		        smsc set esmeerrorcountersenabled <true | false>

		DESCRIPTION
						The SMSC can be configured to calculate total number of errors occurred in SMSC per ESME in different logical units.
						If we set esmeerrorcountersenabled to true then every SMSC error will be counted towards the value of corresponding ESME counter.
						ESME counters are only available in Scheduler, SMPP IN and SMPP OUT logical units.
						This value can be overridden on an ESME level for particular ESME. Hence, if esmeerrorcountersenabled is set to true globally, but ESME "testEsme" has this setting set to false, this counter will be turned off for this ESME.
						If esmeerrorcountersenabled is set to false then errors won't be counted per every ESME, but it can still be enabled for particular ESME(s).
						Default value: false

		EXAMPLES
			smsc set esmeerrorcountersenabled false
		----

		[[_esmeerrorcountersenabled_gui]]
		==== Using GUI

		.Procedure: Set 'esmeerrorcountersenabled'  using the GUI
		. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
		. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
		  Click on the 'Counters'..
		. You can specify if the total numbers of SMSC errors per ESME should be calculated for every ESME by choosing 'true' for the field 'ESME Error Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
		. You must click on the button 'Apply Changes' at the top of the window to save your settings.
		  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


	[[_set_sessionerrorcountersenabled]]
	=== SMSC Session Error Counters

	[[_set_sessionerrorcountersenabled_cli]]
	==== Using CLI

	You can set the value 'SMSC Session Error Counters Enabled' to true/false by entering the command `smsc set sessionerrorcountersenabled`.
	You can verify new value then by issuing the command `smsc get sessionerrorcountersenabled` which will display the value set for this property.

	----

	Name
					smsc set sessionerrorcountersenabled

	SYNOPSIS
					smsc set sessionrrorcountersenabled <true | false>

	DESCRIPTION
					The SMSC can be configured to calculate total number of errors occurred in SMSC per session in different logical units.
					If we set sessionerrorcountersenabled to true then every SMSC error will be counted towards the value of corresponding Session Error counter.
					Session counters are only available in SMPP IN and SMPP OUT logical units.
					This value can be overridden on an ESME level for particular ESME. Hence, if sessionerrorcountersenabled is set to true globally, but ESME "testEsme" has this setting set to false, this counter will be turned off for all sessions with this ESME.
					If sessionerrorcountersenabled is set to false then errors won't be counted per every session, but it can still be enabled for particular ESME(s).
					Default value: false

	EXAMPLES
		smsc set sessionerrorcountersenabled false
	----

	[[_sessionerrorcountersenabled_gui]]
	==== Using GUI

	.Procedure: Set 'sessionerrorcountersenabled'  using the GUI
	. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
	. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
		Click on the 'Counters'..
	. You can specify if the total number of SMSC errors per session should be calculated for every session by choosing 'true' for the field 'Session Error Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
	. You must click on the button 'Apply Changes' at the top of the window to save your settings.
		If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


			[[_set_mprocerrorcountersenabled]]
			=== SMSC MProc Error Counters

			[[_set_mprocerrorcountersenabled_cli]]
			==== Using CLI

			You can set the value 'SMSC MProc Error Counters Enabled' to true/false by entering the command `smsc set mprocerrorcountersenabled`.
			You can verify new value then by issuing the command `smsc get mprocerrorcountersenabled` which will display the value set for this property.

			----

			Name
						smsc set mprocerrorcountersenabled

			SYNOPSIS
						smsc set _set_mprocerrorcountersenabled <true | false>

			DESCRIPTION
						The SMSC can be configured to calculate total number of errors occurred in SMSC per every MProc rule.
						If we set mprocerrorcountersenabled to true then every SMSC error that occurred while processing MProc rules will be counted towards the value of respective MProc Error counter.
						This value can be overridden on an MProc rule level. Hence, if mprocerrorcountersenabled is set to true globally, but certain MProc rule has has this setting set to false, this counter will be turned off for this particular MProc rule.
						If mprocerrorcountersenabled is set to false then errors won't be counted per every MProc rule, but it can still be enabled for particular MProc rules..
						Default value: false

			EXAMPLES
				smsc set mprocerrorcountersenabled false
			----

			[[_mprocerrorcountersenabled_gui]]
			==== Using GUI

			.Procedure: Set 'mprocerrorcountersenabled'  using the GUI
			. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
			. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
				Click on the 'Counters'..
			. You can specify if the total number of SMSC errors per MProc rule should be calculated for every MProc rule by choosing 'true' for the field 'SESSION ERROR Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
			. You must click on the button 'Apply Changes' at the top of the window to save your settings.
				If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


[[_set_globalmaintenancecountersenabled]]
=== SMSC Global Maintenance Counters

[[_set_globalmaintenancecountersenabled_cli]]
==== Using CLI

You can set the value 'SMSC Global Maintenance Counters Enabled' to true/false by entering the command `smsc set globalmaintenancecountersenabled`.
You can verify new value then by issuing the command `smsc get globalmaintenancecountersenabled` which will display the value set for this property.

----

Name
        smsc set globalmaintenancecountersenabled

SYNOPSIS
        smsc set globalmaintenancecountersenabled <true | false>

DESCRIPTION
				The SMSC can be configured to calculate sizes of various SMSC queues and other maintenance indicators, namely: Total of all Request Queues sizes, Total of all Response Queue sizes, size of queue of schedule connects to client-type ESMEs, size of queue of enquire_link packets to be sent to client-type ESMEs, total, successful and failed number of reconnects to ESMEs, total number of ESMEs in Started state which are disconnected and SLEE event queue size.
				If we set globalmaintenancecountersenabled to true then SMSC will calculate counters for above mentioned queues and numbers.
				If globalmaintenancecountersenabled is set to false then those counters won't be calculated.
				Default value: false

EXAMPLES
	smsc set globalmaintenancecountersenabled false
----

[[_globalmaintenancecountersenabled_gui]]
==== Using GUI

.Procedure: Set 'globalmaintenancecountersenabled'  using the GUI
. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
  Click on the 'Counters'..
. You can specify if the global maintenance counters should be enabled by choosing 'true' for the field 'Global Maintenance Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
. You must click on the button 'Apply Changes' at the top of the window to save your settings.
  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


	[[_set_clustermaintenancecountersenabled]]
	=== SMSC Cluster Maintenance Counters

	[[_set_clustermaintenancecountersenabled_cli]]
	==== Using CLI

	You can set the value 'SMSC Cluster Maintenance Counters Enabled' to true/false by entering the command `smsc set clustermaintenancecountersenabled`.
	You can verify new value then by issuing the command `smsc get clustermaintenancecountersenabled` which will display the value set for this property.

	----

	Name
	        smsc set clustermaintenancecountersenabled

	SYNOPSIS
	        smsc set clustermaintenancecountersenabled <true | false>

	DESCRIPTION
					The SMSC can be configured to calculate sizes of various SMSC queues and other maintenance indicators per cluster, namely: Total of Request Queues sizes for all ESMEs in a cluster, Total of Response Queue sizes for all ESMEs in a cluster, size of queue of schedule connects to client-type ESMEs in a cluster, size of queue of enquire_link packets to be sent to client-type ESMEs in a cluster, total, successful and failed number of reconnects to ESMEs in a cluster and total number of ESMEs in a cluster that are in Started state but disconnected.
					If we set clustermaintenancecountersenabled to true then SMSC will calculate counters for above mentioned queues and numbers for every cluster.
					If clustermaintenancecountersenabled is set to false then those counters won't be calculated.
					Default value: false

	EXAMPLES
		smsc set clustermaintenancecountersenabled false
	----

	[[_clustermaintenancecountersenabled_gui]]
	==== Using GUI

	.Procedure: Set 'clustermaintenancecountersenabled'  using the GUI
	. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
	. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
	  Click on the 'Counters'..
	. You can specify if the cluster maintenance counters should be enabled by choosing 'true' for the field 'Cluster Error Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
	. You must click on the button 'Apply Changes' at the top of the window to save your settings.
	  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.


		[[_set_esmemaintenancecountersenabled]]
		=== SMSC ESME Maintenance Counter

		[[_set_esmemaintenancecountersenabled_cli]]
		==== Using CLI

		You can set the value 'SMSC ESME Maintenance Counters' to true/false by entering the command `smsc set esmemaintenancecountersenabled`.
		You can verify new value then by issuing the command `smsc get esmemaintenancecountersenabled` which will display the value set for this property.

		----

		Name
	        smsc set esmemaintenancecountersenabled

		SYNOPSIS
	        smsc set esmemaintenancecountersenabled <true | false>

		DESCRIPTION
					The SMSC can be configured to maintain sizes of two SMSC queues for every ESME, namely: Request Queues size and Response Queue size.
					If we set esmermaintenancecountersenabled to true then SMSC will calculate counters for above mentioned queues for every ESME.
					This value can be overridden on an ESME level for particular ESMEs. Hence, if esmemaintenancecountersenabled is set to true, but ESME "testEsme" has this setting as false, this counter will be turned off for this ESME.
					If esmermaintenancecountersenabled is set to false then those counters won't be calculated.
					Default value: false

		EXAMPLES
			smsc set esmemaintenancecountersenabled false
		----

		[[_esmemaintenancecountersenabled_gui]]
		==== Using GUI

		.Procedure: Set 'esmemaintenancecountersenabled'  using the GUI
		. In the GUI Management Console for SMSC Gateway, click on 'Server Settings' on the left panel.
		. The main panel will display the existing Settings, segregated into nine horizontal tabs: Properties, SS7 Settings, Cassandra, Scheduler, Diameter, Processing, CDR, Home Routing and Counters.
		  Click on the 'Counters'..
		. You can specify if the esme maintenance counters should be enabled by choosing 'true' for the field 'ESME Maintenance Counters Enabled'. For more details of this parameter, please refer to the description of the CLI command in the preceding section.
		. You must click on the button 'Apply Changes' at the top of the window to save your settings.
		  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below.
