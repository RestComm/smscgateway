 ...
 * 
 */
public final class PersistenceRAInterfaceProxy extends DBOperations implements PersistenceRAInterface, InitializingBean {

    private static final Log LOG = LogFactory.getLog(PersistenceRAInterfaceProxy.class);

    private String itsIpAddress;
    private String itsKeySpaceName;
    private boolean oldShortMessageDbFormat;

    /**
     * Instantiates a new persistence RA interface proxy.
     */
    public PersistenceRAInterfaceProxy() {
        super();
    }

    @Override
    public void afterPropertiesSet() throws PersistenceException {
        if (itsIpAddress == null) {
            throw new IllegalStateException("IpAddress is not set.");
        }
        if (itsKeySpaceName == null) {
            throw new IllegalStateException("KeySpaceName is not set.");
        }
        testCassandraAccess();
        start();
    }
    ...
    @Override
    public boolean do_scheduleMessage(final Sms sms, final long dueSlot, final ArrayList<Sms> lstFailured,
            final boolean fastStoreAndForwordMode, final boolean removeExpiredValidityPeriod) throws PersistenceException {
        return super.do_scheduleMessage(sms, dueSlot, lstFailured, fastStoreAndForwordMode, removeExpiredValidityPeriod);
    }

    @Override
    public PreparedStatementCollection getStatementCollection(final Date dt) throws PersistenceException {
        return super.getStatementCollection(dt);
    }

    @Override
    public TargetAddress obtainSynchroObject(final TargetAddress ta) {
        return SmsSetCache.getInstance().addSmsSet(ta);
    }

    @Override
    public void releaseSynchroObject(final TargetAddress ta) {
    ...
    public String getKeySpaceName() {
        return itsKeySpaceName;
    }

    public void start() throws PersistenceException {
        super.start(itsIpAddress, 9042, itsKeySpaceName, 60, 60, 60 * 10, 1, 10000000000L);
    }

    public void startMinMaxMessageId(final long minMessageId, final long maxMessageId) throws PersistenceException {
        super.start(itsIpAddress, 9042, itsKeySpaceName, 60, 60, 60 * 10, minMessageId, maxMessageId);
    }

    private void testCassandraAccess() {
        final Cluster cluster = Cluster.builder().addContactPoint(itsIpAddress).build();
        try {
            final Metadata metadata = cluster.getMetadata();

            for (final Host host : metadata.getAllHosts()) {
                LOG.info(String.format("Datacenter: %s. Host: %s. Rack: %s.", host.getDatacenter(), host.getAddress(),
                        host.getRack()));
            }

            final Session session = cluster.connect();

            session.execute("USE \"" + itsKeySpaceName + "\"");
            ...
            final ProtocolVersion protVersion = DBOperations.getProtocolVersion(cluster);
            if (protVersion == ProtocolVersion.V1) {
                throw new IllegalStateException("We do not support cassandra databse 1.2 anymore.");
            ...
            final Date dt2 = new Date(new Date().getTime() + 1000 * 60 * 60 * 24);
            final Date dt3 = new Date(new Date().getTime() - 1000 * 60 * 60 * 24);
    ...
    public SmsProxy obtainArchiveSms(final long dueSlot, final String dstDigits, final UUID dbId)
            throws PersistenceException, IOException {

        final PreparedStatement ps = session.prepare("select * from \"" + Schema.FAMILY_MESSAGES + this.getTableName(dueSlot)
                + "\" where \"" + Schema.COLUMN_ADDR_DST_DIGITS + "\"=? and \"" + Schema.COLUMN_ID + "\"=?;");
        final BoundStatement boundStatement = new BoundStatement(ps);
        boundStatement.bind(dstDigits, dbId);
        final ResultSet result = session.execute(boundStatement);

        final Row row = result.one();
        final SmsSet smsSet = createSms(row, null, true, true, true, true, true, false);
        if (smsSet == null)
            return null;

        final SmsProxy res = new SmsProxy();
    ...
    public int checkSmsExists(final long dueSlot, final String targetId) throws PersistenceException {
        try {
            final String s1 = "select \"ID\" from \"SLOT_MESSAGES_TABLE" + this.getTableName(dueSlot)
                    + "\" where \"DUE_SLOT\"=? and \"TARGET_ID\"=?;";
            final PreparedStatement ps = session.prepare(s1);
            final BoundStatement boundStatement = new BoundStatement(ps);
    ...
    public Sms obtainLiveSms(final long dueSlot, final String targetId, final UUID id) throws PersistenceException {
        try {
            final String s1 = "select * from \"SLOT_MESSAGES_TABLE" + this.getTableName(dueSlot)
                    + "\" where \"DUE_SLOT\"=? and \"TARGET_ID\"=? and \"ID\"=?;";
            final PreparedStatement ps = session.prepare(s1);
            final BoundStatement boundStatement = new BoundStatement(ps);
    ...
    public void setIpAddress(final String ipAddress) {
        itsIpAddress = ipAddress;
    }

    /**
     * Sets the old short message database format.
     *
     * @param aValue the new old short message database format
     */
    public void setOldShortMessageDbFormat(final boolean aValue) {
        oldShortMessageDbFormat = aValue;
    }

    /**
     * Sets the key space name.
     *
     * @param aKeySpaceName the new key space name
     */
    public void setKeySpaceName(final String aKeySpaceName) {
        itsKeySpaceName = aKeySpaceName;
    }

    /**
     * Truncate tables that are reused between tests.
     */
    public void clear() {
        final DateFormat df = new SimpleDateFormat("yyyy_MM_dd");
        final String t1r = "DST_SLOT_TABLE_" + df.format(new Date());
        final String t2r = "SLOT_MESSAGES_TABLE" + df.format(new Date());
        final Session s = getSession();
        final ResultSet rs = s.execute("SELECT table_name FROM system_schema.tables WHERE keyspace_name = 'RestCommSMSC'");
        for (final Row r : rs.all()) {
            final String t = r.getString("table_name");
            if (t.equals(t1r)) {
                s.execute("TRUNCATE \"RestCommSMSC\".\"" + t + "\"");
            }
            if (t.equals(t2r)) {
                s.execute("TRUNCATE \"RestCommSMSC\".\"" + t + "\"");
            }
     ...
     */
    @Override
    public long c2_getCurrentSlotTable(final int key) throws PersistenceException {
        return super.c2_getCurrentSlotTable(key);
    }

    @Override
    protected void addSmsFields(final StringBuilder sb) {
    ...
    @Override
    protected String[] getLiveTableListAsNames(final String keyspace) {
        return super.getLiveTableListAsNames(keyspace);
    }

    private void doTrauncateTables(final Session session, final String tName) {
        PreparedStatement ps;
        BoundStatement boundStatement;
