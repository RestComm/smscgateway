...
public class DBOperations {
    ...
    public void start(String hosts, int port, String keyspace, String user, String password, int secondsForwardStoring,
        ...
        if (protVersion == ProtocolVersion.V1) {
            ...
            logger.error(
                    "We do not support more cassandra database 1.2. You need to install cassandra database V2.0, 2.1 or 3.0");
            this.started = true;
            return;
        }
        ...
        String sa = "SELECT \"" + Schema.COLUMN_NEXT_SLOT + "\" FROM \"" + Schema.FAMILY_CURRENT_SLOT_TABLE + "\" where \""
                + Schema.COLUMN_ID + "\"=?;";
        ...
        getSmppSmsRoutingRule = session.prepare("select * from \"" + Schema.FAMILY_SMPP_SMS_ROUTING_RULE + "\" where \""
                + Schema.COLUMN_ADDRESS + "\"=? and \"" + Schema.COLUMN_NETWORK_ID + "\"=?;");
        getSipSmsRoutingRule = session.prepare("select * from \"" + Schema.FAMILY_SIP_SMS_ROUTING_RULE + "\" where \""
                + Schema.COLUMN_ADDRESS + "\"=? and \"" + Schema.COLUMN_NETWORK_ID + "\"=?;");

        updateSmppSmsRoutingRule = session
                .prepare("INSERT INTO \"" + Schema.FAMILY_SMPP_SMS_ROUTING_RULE + "\" (\"" + Schema.COLUMN_ADDRESS + "\", \""
                        + Schema.COLUMN_NETWORK_ID + "\", \"" + Schema.COLUMN_CLUSTER_NAME + "\") VALUES (?, ?, ?);");
        updateSipSmsRoutingRule = session
                .prepare("INSERT INTO \"" + Schema.FAMILY_SIP_SMS_ROUTING_RULE + "\" (\"" + Schema.COLUMN_ADDRESS + "\", \""
                        + Schema.COLUMN_NETWORK_ID + "\", \"" + Schema.COLUMN_CLUSTER_NAME + "\") VALUES (?, ?, ?);");

        deleteSmppSmsRoutingRule = session.prepare("delete from \"" + Schema.FAMILY_SMPP_SMS_ROUTING_RULE + "\" where \""
                + Schema.COLUMN_ADDRESS + "\"=? and \"" + Schema.COLUMN_NETWORK_ID + "\"=?;");
        ...
        getSmppSmsRoutingRulesRange2 = session
                .prepare("select * from \"" + Schema.FAMILY_SMPP_SMS_ROUTING_RULE + "\"  LIMIT " + row_count + ";");

        getSipSmsRoutingRulesRange = session.prepare("select * from \"" + Schema.FAMILY_SIP_SMS_ROUTING_RULE
                + "\" where token(\"" + Schema.COLUMN_ADDRESS + "\") >= token(?) LIMIT " + row_count + ";");
        getSipSmsRoutingRulesRange2 = session
                .prepare("select * from \"" + Schema.FAMILY_SIP_SMS_ROUTING_RULE + "\"  LIMIT " + row_count + ";");

        getStoredMessagesCounter = session
                .prepare("SELECT * FROM \"" + Schema.FAMILY_PENDING_MESSAGES + "\" WHERE \"" + Schema.COLUMN_DAY + "\" = ?");
        getSentMessagesCounter = session
                .prepare("SELECT * FROM \"" + Schema.FAMILY_PENDING_MESSAGES + "\" WHERE \"" + Schema.COLUMN_DAY + "\" = ?");
    ...
    /**
     * Set a new due_slop that SMSC "is processing now" and store it into the database
     */
    ...
    /**
     * Returns a next messageId Every MESSAGE_ID_LAG messageId will be stored at cassandra database
     */
    ...
    /**
     * Checking if dueSlot is possible to use for writing (it is not too soon). if possible - returns dueSlot if not - returns
     * next possible dueSlot
     */
    ...
    /**
     * Checking if due_slot is not in writing state now Returns true if due_slot is not in writing now
     */
    ...
    /**
     * scheduler a message into a dueSlot that is already scheduled for all messages for this targetId or for a next available
     * if no dueSlot has been scheduled for targetId
     */
    ...
    private void c2_scheduleMessage_ReschedDueSlot(Sms sms, boolean fastStoreAndForwordMode, long preferredDueSlot,
            ArrayList<Sms> lstFailured, boolean removeExpiredValidityPeriod) throws PersistenceException {
        if (sms.getStored()) {
            long dueSlot = 0;
            PreparedStatementCollection[] lstPsc = this.c2_getPscList();
            boolean done = false;
     ...
     */
    public void c2_scheduleMessage_NewDueSlot(Sms sms, long dueSlot, ArrayList<Sms> lstFailured,
            boolean fastStoreAndForwordMode) throws PersistenceException {
    ...
    public void c2_createRecordArchive(Sms sms, String dlvMessageId, String dlvDestId, boolean deliveryReceipts,
        ...
        if (incomingDeliveryReceipts && !MessageUtil.isDeliveryReceipt(sms) && dlvMessageId != null && dlvMessageId.length() > 0
                && dlvDestId != null) {
            try {
    ...
    private void setSmsFields(Sms sms, long dueSlot, BoundStatement boundStatement, boolean archive,
        ...
        if (!archive) {
            ...
            if (sms.getSmsSet().getLocationInfoWithLMSI() != null
                ...
                boundStatement.setString(Schema.COLUMN_NNN_DIGITS,
                        sms.getSmsSet().getLocationInfoWithLMSI().getNetworkNodeNumber().getAddress());
                boundStatement.setInt(Schema.COLUMN_NNN_AN,
                        sms.getSmsSet().getLocationInfoWithLMSI().getNetworkNodeNumber().getAddressNature().getIndicator());
                boundStatement.setInt(Schema.COLUMN_NNN_NP,
                        sms.getSmsSet().getLocationInfoWithLMSI().getNetworkNodeNumber().getNumberingPlan().getIndicator());
            } else {
        ...
        if (smsSet == null) {
            ...
            if (destAddr == null || destAddrTon == -1 || destAddrNpi == -1) {
                throw new PersistenceException(
                        "destAddr or destAddrTon or destAddrNpi is absent for ID='" + sms.getDbId() + "'");
            }
        ...
        try {
            try {
                ...
                String sa = "SELECT \"" + Schema.COLUMN_ADDRESS + "\" FROM \"" + Schema.FAMILY_SMPP_SMS_ROUTING_RULE
                        + "\" where \"" + Schema.COLUMN_ADDRESS + "\"='' and \"" + Schema.COLUMN_NETWORK_ID + "\"=0;";
                PreparedStatement ps = session.prepare(sa);
            } catch (InvalidQueryException e) {
                StringBuilder sb = new StringBuilder();
                sb.append("CREATE TABLE \"");
    ...
    /**
     * Updates the SMPP_SMS_ROUTING_RULE table, sets the CLUSTER_NAME for passed ADDRESS
     * 
    ...
    public void c2_updateSmppSmsRoutingRule(DbSmsRoutingRule dbSmsRoutingRule) throws PersistenceException {
        try {
            ...
            boundStatement.bind(dbSmsRoutingRule.getAddress(), dbSmsRoutingRule.getNetworkId(),
                    dbSmsRoutingRule.getClusterName());
            session.execute(boundStatement);
        } catch (Exception e) {
            String msg = "Failed to updateSmppSmsRoutingRule for address=" + dbSmsRoutingRule.getAddress() + " networkId="
                    + dbSmsRoutingRule.getNetworkId() + " !";
            throw new PersistenceException(msg, e);
        }
    }

    /**
     * Updates the SIP_SMS_ROUTING_RULE table, sets the CLUSTER_NAME for passed ADDRESS
     * 
     * @param dbSmsRoutingRule
     * @throws PersistenceException
     */
    public void c2_updateSipSmsRoutingRule(DbSmsRoutingRule dbSmsRoutingRule) throws PersistenceException {
        try {
            ...
            String msg = "Failed to updateSipSmsRoutingRule for address=" + dbSmsRoutingRule.getAddress() + " networkId="
                    + dbSmsRoutingRule.getNetworkId() + " !";
            throw new PersistenceException(msg, e);
        }
    }
    ...
    /**
     * Deleting live tables (DST_SLOT_TABLE_YYYY_MM_DD and SLOT_MESSAGES_TABLE_YYYY_MM_DD) for a defined date. Before deleting
     * checking is made for we can not delay table for future, today and 2 days before
     * 
     * @param dt Date for a table
     * @return true: success or perm failure, false: temporary failure, we need to retry
     */
    ...
    /**
     * Deleting live table (MESSAGES) for a defined date. Before deleting checking is made for we can not delay table for
     * future, today and 2 days before
     *
     * @param dt Date for a table
     * @return true: success or perm failure, false: temporary failure, we need to retry
     */
    ...
    private CheckDeletingDateResult checkDeletingDate(Date dt, String taskName) {
        ...
        if (procDate.getYear() != curDate.getYear() || procDate.getMonth() != curDate.getMonth()
                || procDate.getDate() != curDate.getDate()) {
            logger.warn(
                    "Rejected an attempt of dropping of " + taskName + " for old message data are still in processing: " + dt);
