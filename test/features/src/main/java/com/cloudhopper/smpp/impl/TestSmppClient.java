 ...
 */
public class TestSmppClient implements SmppClient {
     ...
     * monitorExecutors. The maximum number of IO worker threads across any client sessions created with this SmppClient will be
     * Runtime.getRuntime().availableProcessors(). An Executors.newCachedDaemonThreadPool will be used for IO worker threads.
     */
    public TestSmppClient(String localHost, int localPort) {
        this(DaemonExecutors.newCachedDaemonThreadPool(), localHost, localPort);
    }
     ...
     * 
     * @param executor The executor that IO workers will be executed with. An Executors.newCachedDaemonThreadPool() is
     *        recommended. The max threads will never grow more than expectedSessions if NIO sockets are used.
     */
    public TestSmppClient(ExecutorService executors, String localHost, int localPort) {
        this(executors, Runtime.getRuntime().availableProcessors(), localHost, localPort);
    }
     ...
     * @param executor The executor that IO workers will be executed with. An Executors.newCachedDaemonThreadPool() is
     *        recommended. The max threads will never grow more than expectedSessions if NIO sockets are used.
     * @param expectedSessions The max number of concurrent sessions expected to be active at any time. This number controls the
     *        max number of worker threads that the underlying Netty library will use. If processing occurs in a sessionHandler
     *        (a blocking op), be <b>VERY</b> careful setting this to the correct number of concurrent sessions you expect.
     */
    public TestSmppClient(ExecutorService executors, int expectedSessions, String localHost, int localPort) {
        this(executors, expectedSessions, null, localHost, localPort);
    }
     ...
     *        max number of worker threads that the underlying Netty library will use. If processing occurs in a sessionHandler
     *        (a blocking op), be <b>VERY</b> careful setting this to the correct number of concurrent sessions you expect.
     * @param monitorExecutor The scheduled executor that all sessions will share to monitor themselves and expire requests. If
    ...
    public SmppSession bind(SmppSessionConfiguration config) throws SmppTimeoutException, SmppChannelException,
            SmppBindException, UnrecoverablePduException, InterruptedException {
    ...
    protected void doBind(DefaultSmppSession session, SmppSessionConfiguration config, SmppSessionHandler sessionHandler)
    ...
    protected DefaultSmppSession doOpen(SmppSessionConfiguration config, SmppSessionHandler sessionHandler)
    ...
    protected DefaultSmppSession createSession(Channel channel, SmppSessionConfiguration config,
        ...
        if (config.getWriteTimeout() > 0) {
            WriteTimeoutHandler writeTimeoutHandler = new WriteTimeoutHandler(writeTimeoutTimer, config.getWriteTimeout(),
    ...
    protected Channel createConnectedChannel(String host, int port, long connectTimeoutMillis)
        ...
        /*
         * Wait until the connection is made successfully. According to the netty documentation it is bad to use
         * .await(timeout). Instead b.setOption("connectTimeoutMillis", 10000); should be used. See
         * http://netty.io/3.9/api/org/jboss/netty/channel/ChannelFuture.html It turns out that under certain unknown
         * circumstances the connect waits forever: https://github.com/twitter/cloudhopper-smpp/issues/117 That's why the future
         * is canceled 1 second after the specified timeout. This is a workaround and hopefully not needed after the switch to
         * netty 4.
         */
        if (!connectFuture.await(connectTimeoutMillis + 1000)) {
            ...
            logger.error("connectFuture: isCanceled {} isDone {} isSuccess {: }" + isCanceled + ", " + connectFuture.isDone()
        ...
        if (connectFuture.isCancelled()) {
            ...
            if (connectFuture.getCause() instanceof org.jboss.netty.channel.ConnectTimeoutException) {
                throw new SmppChannelConnectTimeoutException("Unable to connect to host [" + host + "] and port [" + port
                        + "] within " + connectTimeoutMillis + " ms", connectFuture.getCause());
            } else {
                throw new SmppChannelConnectException("Unable to connect to host [" + host + "] and port [" + port + "]: "
                        + connectFuture.getCause().getMessage(), connectFuture.getCause());
            }
        }
